      ...
      <a-col>
        ...
        <div v-if="actionButton" class="action-button">
          <a-button-group v-if="buttonPermission('add')">
            ...
            <!--<a-button icon="plus" @click="addProductDialog">添加商品</a-button>-->
...
<script>
  ...
  import NbcioSelectList from '@/components/jeecgbiz/NbcioSelectList'

  // 行高，需要在实例加载完成前用到
  let rowHeight = 61

  export default {
    name: 'NbcioEditableTable',
    components: { JDate, Draggable, JInputPop, JFilePop, NbcioSelectList },
    provide() {
      return {
    ...
    methods: {
        ...
        values.forEach(item => {
          ...
          for (let newValueKey in newValues) {
            ...
            if (newValues.hasOwnProperty(newValueKey)) {
              ...
              if (!edited) {
                console.warn(`NbcioEditableTable.setValues：没有找到"${newValueKey}"列`)
              }
            }
          }
        })
      ...
      handleClickAdd() {
        this.$emit('beforeAdd')
        console.log("handleClickAdd this.isAdd=",this.isAdd)
        if (this.isAdd) {
      ...
      handleChangePopupGoodsCommon(value, id, row, column,index) {
        ...
        // 触发valueChange 事件
        this.elemValueChange(FormTypes.popupGoods, row, column, value)
      },
