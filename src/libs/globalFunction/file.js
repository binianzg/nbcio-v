...
const fileFunction = {
	 ...
	 */
	getMinImgStream(row) {
		return window._CONFIG['domianURL'] + `/nd/ndStorage/preview?userFileId=${
			row.userFileId
	...
	getDownloadFilePath(row) {
		return window._CONFIG['domianURL'] + `/nd/ndStorage/downloadfile?userFileId=${
			row.userFileId
		}&shareBatchNum=${
			row.shareBatchNum == null ? '' : row.shareBatchNum
		}&extractionCode=${row.extractionCode == null ? '' : row.extractionCode}
	...
	createFileOnlineByOffice(data) {
		let fileUrl = `${location.protocol}//${location.host}/nd`
	 ...
	 */
	setFileImg(file) {
    console.log("setFileImg file",file)
		if (file.isDir === 1) {
			//  文件夹
    ...
    console.log("handleFileNameClick row=",row)
		// 如果当前文件在回收站中，则不允许预览
      ...
      console.log("router.currentRoute=",router.currentRoute)
			if (router.currentRoute.name === 'Share') {
				...
				// 回收站页面不允许打开文件夹
				// 网盘页面
        console.log("fileType",router.currentRoute.query.fileType)
				router.push({
          name: 'netdiskfile',
					query: {
		...
		// 若是文件，则进行相应的预览
			...
			if ([...officeFileType].includes(row.extendName.toLowerCase())) {
				//this.getFileOnlineViewPathByOffice(row) //目前没用onlyOffice
