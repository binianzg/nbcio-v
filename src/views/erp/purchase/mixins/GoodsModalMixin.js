...
export const GoodsModalMixin = {
  ...
  methods: {
    ...
    changeAddedColumn(info) { 
      ...
      getAction(this.url.getbyid,param).then((res) => {
        ...
        if (res && res.code === 200) {
          ...
          console.log("changeAddedColumn columns",this.erpInSheetDetailTable.columns)
          this.$refs.erpInSheetDetail.setValues([
            {
              rowKey: info.id, //行的id
    ...
    changeColumnShow(info) { 
      if(info.name) {
        this.changeFormTypes(this.erpInSheetDetailTable.columns, 'goodsName', 1)
      }
      if(info.code) {
        this.changeFormTypes(this.erpInSheetDetailTable.columns, 'goodsCode', 1)
      }
    },
    //改变字段的状态，1-显示 0-隐藏
          ...
          getAction(this.url.getbyid,param).then((res) => {
            ...
            if (res && res.code === 200) {
              ...
              console.log("onValueChange mList,value",mList,value)
                this.$refs.erpInSheetDetail.getValues((error, values) => {
                  values.pop()  //移除最后一行数据
                  let mArr = values
                  ...
                  this.erpInSheetDetailTable.dataSource = mArr
    ...
    changeColumnHide() {
      //this.changeFormTypes(this.erpInSheetDetailTable.columns, 'goodname', 0)
    },
    //转为商品对象
    parseInfoToObj(mInfo) {
      return {
        ...
        orderNum: mInfo.num,
        oriPrice: mInfo.purchasePrice,
        taxPrice: mInfo.purchasePrice,
        isGift: 0,
        taxRate: 17,
