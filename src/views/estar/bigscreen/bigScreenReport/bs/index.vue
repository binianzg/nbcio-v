...
<script>
import { reportShareDetailByCode } from "@/api/bigscreen/reportShare";
...
export default {
  ...
  methods: {
    async handleOpen() {
      ...
      const { code, result } = await reportShareDetailByCode(shareCode);
      if (code != "200") return;
      this.reportCode = result.reportCode;
      this.sharePassword = result.sharePassword;
      this.shareToken = result.shareToken;
      console.log("handleOpen result=",result);
