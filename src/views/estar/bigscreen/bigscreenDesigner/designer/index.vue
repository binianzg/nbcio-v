...
import "@/assets/bs/iconfont/iconfont.css";
...
export default {
  ...
  data() {
    return {
      ...
      // 工作台大屏画布，保存到表bs_report_dashboard中
      dashboard: {
        id: null,
        title: "", // 大屏页面标题
        width: 1920, // 大屏设计宽度
  ...
  mounted() {
    ...
    console.log("mounted widgetTools",this.widgetTools);
  },
  methods: {
    /**
     * @description: 恢复
    ...
    async initEchartData() {
      ...
      const { code, result } = await detailDashboard(reportCode);
      if (code != 200) return;
      console.log("initEchartData result=",result);
