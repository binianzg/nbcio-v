...
import { getFilePreview, modifyFileContent } from '@/api/netdisk/file.js'
import Vue from 'vue'
...
export default {
	...
	computed: {
		// 屏幕宽度
		screenWidth() {
			return store.state.netdisk.screenWidth
		},
		// 是否修改
		isModify() {
	...
	watch: {
		...
		visible(val) {
			if (val) {
				...
				this.codeMirrorOptions.theme =
					localStorage.getItem('estar_file_codemirror_theme') || 'default'
				this.getCodeText()
				// 添加键盘 Esc 事件
				this.$nextTick(() => {
		...
		'codeMirrorOptions.theme'(val) {
			localStorage.setItem('estar_file_codemirror_theme', val)
		}
	},
	methods: {
		/**
