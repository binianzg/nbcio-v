	...
	>
      ...
      >	显示上传菜单
			...
			<el-dropdown class="upload-drop" trigger="hover">
				...
				></el-button>
				<el-dropdown-menu slot="dropdown" v-if="isDisplayUploadMenu">
...
import '@/assets/nd/styles/iconfont/iconfont.css'
...
export default {
	...
	computed: {
    ...mapState({
      showUploadMask: state => state.netdisk.showUploadMask
    }),
		// 上传文件组件参数
		uploadFileParams() {
			return {
		...
		// 被选中的文件列表
		selectedFiles() {
			return this.$store.state.netdisk.selectedFiles
		},
		// 是否批量操作
		isBatchOperation() {
			return this.$store.state.netdisk.isBatchOperation
		},
		// 屏幕宽度
		screenWidth() {
		...
		// 批量下载文件链接
		batchDownloadLink() {
			return window._CONFIG['domianURL'] + `/nd/ndStorage/batchDownloadFile?userFileIds=${this.selectedFiles
				.map((item) => item.userFileId)
				.join(',')}&token=${Vue.ls.get(ACCESS_TOKEN)}`
		}
	},
	watch: {
    // 显示拖拽上传文件遮罩
	...
	methods: {
      ...
      console.log("handleUploadFileBtnClick this.$openDialog=",this.$openDialog)
			this.$openDialog.authWeChat({}).then((res) => {
        console.log("authWeChat res=",res)
				switch (res) {
					case 'confirm': {
					...
					case 'go': {
            console.log("handleUploadFileBtnClick this.$openBox=",this.$openBox)
