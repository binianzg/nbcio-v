                        ...
                        <div class="log-wrap">
                            ...
                            <vue-scroll>
                                <div class="log-list muted">
                                    ...
                                    <div class="list-item"
                                         v-for="log in versionLog" :key="log.id">
                                        <template>
...
<script>
    ...
    import {relativelyTaskTime, relativelyTime} from "@/assets/tw/js/dateTime";
    import {checkResponse} from "@/assets/tw/js/utils";
    import {addVersionTask, changeStatus, removeVersionTask} from "@/api/teamwork/version";
    import {getStagesByProjectId as getTaskStages, getTasks} from "@/api/teamwork/task";
    import {notice} from "@/assets/tw/js/notice";
    ...
    export default {
        ...
        methods: {
            ...
            getVersion() {
                getVersionInfo({versionId: this.id}).then(res => {
                    this.getVersionLog();
            ...
            getVersionTask() {
                getVersionTask({versionId: this.id}).then(res => {
                    this.versionTaskList = res.result;
                });
            },
            getVersionLog() {
                getVersionLog({
                    versionId: this.id, all: this.showMoreVersionLog,
                    pageSize: 5,
                }).then(res => {
                    // this.versionLog = res.result;
            ...
            changeVersionStatus(e) {
                ...
                if (e.key == 3) {
                    ...
                    changeStatus({versionId: this.version.id, status: e.key}).then(res => {
                        this.getVersion();
                    });
                }
            },
            ...
            addVersionTask() {
                addVersionTask({
                    ...
                    notice({title: `成功添加 ${res.result.successTotal} 项发布内容到版本`}, 'notice', 'success');
            ...
            removeVersionTask(task) {
                ...
                this.$confirm({
                    ...
                    cancelText: `再想想`,
                    onOk() {
                        removeVersionTask({taskId: task.id}).then((res) => {
                            const result = checkResponse(res);
                            if (!result) {
                                return false;
                            }
