    ...
    <div class="project-navigation">
      ...
      <section class="nav-body">
        <ul class="nav-wrapper nav nav-underscore pull-left">
          ...
          <li class="actives"><a class="app" data-app="works" @click="$router.push('/estar/teamwork/space/files/' + id)">
              文件</a>
              ...
              <a-list-item class="list-item" :key="index" v-for="(item, index) in files">
                <a-list-item-meta>
                  ...
                  <div slot="title">
                    <a-tooltip :mouseEnterDelay="0.3">
                      ...
                      <a-input :ref="`inputTitle${index}`" :auto-focus="true" v-model="item.title" v-show="item.editing"
                        @pressEnter="onCellChange(item)" @blur="onCellChange(item)"></a-input>
                ...
                <a class="muted" slot="actions">
                  <a-dropdown :trigger="['click']" placement="bottomCenter">
                    ...
                    <a-menu class="field-right-menu" v-clipboard="item.file_url" @click="doFile($event,item.id)"
                      slot="overlay">
...
<script>
  ...
  import {edit, fileList, recycle} from "@/api/teamwork/file";
  ...
  export default {
    ...
    methods: {
      ...
      getFiles(reset = true) {
        ...
        that.requestData.projectId = this.id;
        that.requestData.deleted = 0;
        fileList({projectId: this.id, delete: 0}).then(res => {
          if (reset) {
            this.files = [];
          }
          ...
          that.files = that.files.concat(res.result.list);
          that.pagination.total = res.result.total;
          that.showLoadingMore = that.pagination.total > that.files.length;
          that.loading = false;
